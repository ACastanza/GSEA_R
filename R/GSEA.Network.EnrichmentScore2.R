...
GSEA.Network.EnrichmentScore2 <-
function(gene.list, gene.set, correl.vector = NULL, 
 ...
 loc.vector <- as.complex(loc.vector)^unname(weighted.score.matrix) 

 tag.correl.vector <- abs(correl.vector[tag.loc.vector])^unname(weighted.score.matrix[tag.loc.vector])
